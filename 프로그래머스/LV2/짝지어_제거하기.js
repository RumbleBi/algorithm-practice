...
function solution(s) {
  // 효율성 검증을 위해 홀수인 문자열은 무조건 0 반환하게 얼리리턴.
  if (s.length % 2 != 0) return 0;

  let newArr = s.split("");
  let stack = [];
  // b a a b a a 순으로 배열을 순환하는데 만약 참이면 pop시키고 아니면 el 값을 넣어서 다음 el 과 비교하게 한다.
  // 효율성 검증시 for > forEach > map 순으로 성능차이를 보였다.
  // 또한 삼항연산자 > if문 의 성능차이도 보였다.
  newArr.forEach((el) => {
    stack[stack.length - 1] === el ? stack.pop() : stack.push(el);
  });
  // 배열이 모두 사라지면 1, 배열이 남아있으면 0
